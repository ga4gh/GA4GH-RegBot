...
1. Ingest the bundled synthetic policy text into a dedicated demo store (the full corpus store is untouched):

```bash
python -m src.main --store ./data/regbot_demo_store ingest --path examples/data/sample_ga4gh_policy_stub.txt --reset
```

2. Run a check against the sample consent:

```bash
python -m src.main --store ./data/regbot_demo_store check --consent examples/data/sample_consent_short.txt
```
