...
def _cmd_benchmark(args: argparse.Namespace) -> int:
    ...
    if not chunks:
        print(
            f"No ingested chunks in {bot.store_dir}. Run 'ingest-manifest --reset' first.",
            file=sys.stderr,
        )
        return 1

    gold = load_gold_set(args.gold)
    ks = [int(k) for k in args.ks.split(",") if k.strip()] if args.ks else list(DEFAULT_KS)

    def retrieve(query: str, top_k: int, jurisdiction: Optional[List[str]]) -> List[Dict[str, Any]]:
        return bot.retrieve_relevant_clauses(query, top_k=top_k, jurisdiction=jurisdiction)

    result = evaluate_gold_set(gold, chunks, retrieve, ks=ks, label=args.label)
    ...
    if args.markdown:
        write_markdown_report(
            ...
            header=(
                ...
                "Generated by `python -m src.main benchmark`. "
                "See docs/DESIGN.md §4.2 for metric definitions."
            ),
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    pb = sub.add_parser(
        "benchmark",
        help="Score retrieval against the gold set (Recall@k, Precision@k, MRR).",
    )
    pb.add_argument(
        "--gold",
        default="examples/eval/gold_ga4gh.yaml",
        help="Gold set YAML (default: examples/eval/gold_ga4gh.yaml).",
    )
    pb.add_argument(
        "--ks",
        default=None,
        help="Comma-separated cut-offs (default: 1,3,5,8).",
    )
    pb.add_argument(
        "--label",
        default="baseline",
        help="Run label recorded in the output (e.g. baseline, top_k=12).",
    )
    pb.add_argument(
        "--markdown",
        default=None,
        help="Also write a Markdown table to this path (e.g. docs/eval_results.md).",
    )
    pb.add_argument(
        "--min-recall",
        type=float,
